...
public class test {
	...
	public static void main(String[] args) {
		...
		do {
			...
			for (int i = 0; i < arr.length; i++) {
				for (int j = 0; j < arr.length; j++) {
					if (arr[i][0] == "X" && arr[i][1] == "X" && arr[i][2] == "X") {c++;
					}
					if (arr[0][j] == "X" && arr[1][j] == "X" && arr[2][j] == "X") {c++;
					}
					if (arr[i][0] == "O" && arr[i][1] == "O" && arr[i][2] == "O") {c++;c++;
					}
					if (arr[0][j] == "O" && arr[1][j] == "O" && arr[2][j] == "O") {c++;c++;
					}
					if (arr[0][0] == "X" && arr[1][1] == "X" && arr[2][2] == "X"
					|| arr[0][2] == "X" && arr[1][1] == "X" && arr[2][0] == "X") {c++;
					}
					if (arr[0][0] == "O" && arr[1][1] == "O" && arr[2][2] == "O"
					|| arr[0][2] == "O" && arr[1][1] == "O" && arr[2][0] == "O") {c++;
					}
				}
			}
			if (c == 3) {
				System.out.println("Plater 1 has wonn");
				break;
			}
			if (c == 6) {
