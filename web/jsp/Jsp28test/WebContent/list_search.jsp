...
<body>
	<%
			...
			String condition = request.getParameter("condition");	
 			...
 			System.out.println("list.jsp :"+ condition);
					...
					<td colspan="5" align="center">
						...
						<!-- 검색한 리스트페이징 출력 --> 
 						<!-- 처음 --> <c:choose>
							...
							<c:otherwise>
								<a href="list.do?${cboard}&cboard=${cboard}&search=${search}&condition=${condition}&page=1">[처음 ]</a>
							</c:otherwise>
						</c:choose> <!-- 이전 --> <c:choose>
							<c:when test="${(page.curPage -1) < 1}">
							[ &lt; ]
							...
							<c:otherwise>
								<a
									href="list.do?page=${page.curPage -1}&cboard=${cboard}&search=${search}&condition=${condition}">[
									&lt;]</a>
							</c:otherwise>
						</c:choose> <!-- 개별 페이지 --> <c:forEach var="num" begin="${page.startPage}"
							end="${page.endPage}" step="1">
            				...
            				[${num}] &nbsp;
								...
								<c:otherwise>
									<a
										href="list.do?page=${num}&cboard=${cboard}&search=${search}&condition=${condition}">[${num}]</a> &nbsp;	
            			</c:otherwise>
							</c:choose>
						</c:forEach> <!-- 다음 --> <c:choose>
							<c:when test="${(page.curPage +1 ) > page.totalPage }">
							...
							<c:otherwise>
								<a
									href="list.do?page=${page.totalPage}&cboard=${cboard}&search=${search}&condition=${condition}&page=">[끝]</a>
							</c:otherwise>
						</c:choose>						
						<% 
						 	} else {
